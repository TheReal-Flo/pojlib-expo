...
public final class GL {
    ...
    private static boolean isPojavRenderer() {
    ...
    public static GLCapabilities createCapabilities(boolean forwardCompatible, @Nullable IntFunction<PointerBuffer> bufferFactory) {
        ...
        boolean pojavRenderer = isPojavRenderer();
        if (pojavRenderer) {
            try {
                fixPojavGLContext();
            } catch (Exception e) {
            ...
            apiLog("Using Pojav native probe path for GL capability initialization.");
        }
        ...
        int errorCode = pojavRenderer ? GL_NO_ERROR : callI(GetError);
        ...
        if (pojavRenderer) {
            majorVersion = pojavProbeInt(GL_MAJOR_VERSION);
            minorVersion = pojavProbeInt(GL_MINOR_VERSION);
            if (majorVersion < 3) {
                String versionString = pojavProbeString(GL_VERSION);
            ...
            apiLog(String.format("Pojav GL probe detected version %d.%d", majorVersion, minorVersion));
        } else try (MemoryStack stack = stackPush()) {
            IntBuffer version = stack.ints(0);

            // Try the 3.0+ version query first
            callPV(GL_MAJOR_VERSION, memAddress(version), GetIntegerv);
            int majorVersionError = callI(GetError);
            if (majorVersionError == GL_NO_ERROR && 3 <= (majorVersion = version.get(0))) {
        ...
        if (majorVersion < 3) {
            ...
            String extensionsString = pojavRenderer
                ? pojavProbeString(GL_EXTENSIONS)
                : memASCIISafe(callP(GL_EXTENSIONS, GetString));
            ...
            try (MemoryStack stack = stackPush()) {
                ...
                int extensionCount;
                if (pojavRenderer) {
                    extensionCount = pojavProbeInt(GL_NUM_EXTENSIONS);
                } else {
                    callPV(GL_NUM_EXTENSIONS, memAddress(pi), GetIntegerv);
                    extensionCount = pi.get(0);
                }
                apiLog(String.format("Detected %d GL extensions", extensionCount));

                long GetStringi = pojavRenderer ? NULL : apiGetFunctionAddress(functionProvider, "glGetStringi");
                for (int i = 0; i < extensionCount; i++) {
                    String extension = pojavRenderer
                        ? pojavProbeStringi(GL_EXTENSIONS, i)
                        : memASCII(callP(GL_EXTENSIONS, i, GetStringi));
                    if (extension != null) {
                ...
                int contextFlags;
                if (pojavRenderer) {
                    contextFlags = pojavProbeInt(GL_CONTEXT_FLAGS);
                } else {
                    callPV(GL_CONTEXT_FLAGS, memAddress(pi), GetIntegerv);
                    contextFlags = pi.get(0);
                }
                if ((contextFlags & GL_CONTEXT_FLAG_FORWARD_COMPATIBLE_BIT) != 0) {
                    ...
                    if ((3 < majorVersion || 1 <= minorVersion)) { // OpenGL 3.1+
                        if (3 < majorVersion || 2 <= minorVersion) { // OpenGL 3.2+
                            int profileMask;
                            if (pojavRenderer) {
                                profileMask = pojavProbeInt(GL_CONTEXT_PROFILE_MASK);
                            } else {
                                callPV(GL_CONTEXT_PROFILE_MASK, memAddress(pi), GetIntegerv);
                                profileMask = pi.get(0);
                            }
