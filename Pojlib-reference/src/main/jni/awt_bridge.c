...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    if (dalvikJavaVMPtr == NULL) {
        ...
        class_MainActivity = (*env)->NewGlobalRef(env,(*env)->FindClass(env, "pojlib/PojlibRuntimeHost"));
        method_OpenLink = (*env)->GetStaticMethodID(env, class_MainActivity, "openLink", "(Ljava/lang/String;)V");
        method_OpenPath = (*env)->GetStaticMethodID(env, class_MainActivity, "openPath", "(Ljava/lang/String;)V");
...
JNIEXPORT void JNICALL Java_pojlib_input_AWTInputBridge_nativeSendData(JNIEnv* env, jclass clazz, jint type, jint i1, jint i2, jint i3, jint i4) {
...
JNIEXPORT void JNICALL Java_pojlib_input_AWTInputBridge_nativeClipboardReceived(JNIEnv *env, jclass clazz, jstring clipboardData, jstring clipboardDataMime) {
...
Java_pojlib_input_AWTInputBridge_nativeMoveWindow(JNIEnv *env, jclass clazz, jint xoff, jint yoff) {
    if (runtimeJNIEnvPtr_INPUT == NULL) {
        if (runtimeJavaVMPtr == NULL) {
            return;
        } else {
